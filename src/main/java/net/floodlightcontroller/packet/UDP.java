...
public class UDP extends BasePacket {
    ...
    public static final TransportPort SIP_PORT = TransportPort.of(5060);
    ...
    static {
        ...
        UDP.decodeMap.put(SIP_PORT, SIP.class);
    ...
    protected UDPType udpType = UDPType.UDP;
    ...
    public IPacket deserialize(byte[] data, int offset, int length)
        ...
        if ( this.destinationPort.getPort() == DHCP_SERVER_PORT.getPort()) {
            try {
                this.payload = UDP.decodeMap.get(DHCP_SERVER_PORT).getConstructor().newInstance();
                this.udpType = UDPType.DHCP_SERVER;
            } catch (Exception e) {
                throw new RuntimeException("Failure instantiating class", e);
            }
        } else if (this.sourcePort.getPort() == DHCP_CLIENT_PORT.getPort()) {
            try {
                this.payload = UDP.decodeMap.get(DHCP_CLIENT_PORT).getConstructor().newInstance();
                this.udpType = UDPType.DHCP_CLIENT;
            } catch (Exception e) {
                throw new RuntimeException("Failure instantiating class", e);
            }
        } else if ( this.sourcePort.getPort() == SIP_PORT.getPort() || this.destinationPort.getPort() == SIP_PORT.getPort()) {
        	try {
				this.payload = UDP.decodeMap.get(SIP_PORT).getConstructor().newInstance();
				this.udpType = UDPType.SIP;
			} catch (Exception e) {
				throw new RuntimeException("Failure instantiating class", e);
			}
        } else {
            try
            {
            	// TODO: Implement SIP parser
            	RTP rtp = new RTP();
            	// RTP header init on byte 42
            	rtp.deserialize(data, 42, length);
            	if ( rtp.isRTCP() )
            	{
            		this.udpType = UDPType.RTCP;
            	}
            	else
            	{
            		this.udpType = UDPType.RTP;
            	}
            	this.payload = rtp;
            }
            catch(Exception ex)
	...
	public UDPType getUdpType() {
		return udpType;
	}

	public void setUdpType(UDPType udpType) {
